...
# Initialize with defaults first to ensure variables exist even if imports fail
email_settings = None
conf = None
...
try:
    ...
    logger.error(f"Error importing email config dependencies: {e}", exc_info=True)
    # Set defaults using environment variables directly
    class EmailSettingsFallback:
        def __init__(self):
            self.mail_username = os.getenv("MAIL_USERNAME", "your-email@gmail.com")
            self.mail_password = os.getenv("MAIL_PASSWORD", "your-app-password")
            self.mail_from = os.getenv("MAIL_FROM", "your-email@gmail.com")
            self.mail_port = int(os.getenv("MAIL_PORT", "587"))
            self.mail_server = os.getenv("MAIL_SERVER", "smtp.gmail.com")
            self.mail_from_name = os.getenv("MAIL_FROM_NAME", "Study Buddy")
            self.secret_key = os.getenv("SECRET_KEY", "your-super-secret-key-change-in-production")
    
    email_settings = EmailSettingsFallback()
    conf = None
